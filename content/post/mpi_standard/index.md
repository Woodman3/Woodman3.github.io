...
在这先提一嘴，MPI中有一类模糊类型，被称之为`opaque type`。指的是无法被用户直接访问，只能通过句柄访问，其大小和形状对用户不可见的类型，如`groups` `datatypes` `communicators`等。如果一个模糊类型被作为参数，那么他的参数说明为OUT或INOUT，哪怕其句柄并未被更新。
