...
后面在构建的过程中，我又发现求解器在调用pardiso求解稀疏矩阵的时候在重排序这一部分会有异常，具体表现为分解出来的L矩阵非零元非常多，几乎满了，进而导致内存占用过大。

这个问题非常诡异，我以为是矩阵问题或者我写的warpper有问题。但是经过验证发现矩阵本身没有任何问题，把矩阵打印出来的可以在warpper的单测里正常求解。

eigen内部对于pardiso的接口暴露的不够多，如果想要把pardiso的报错打开要么自己写个子类来改msglvl，要么去改eigen的源码。我决定还是直接去改eigen的源码实在一些，在对比最终的pardiso的调用后，我发现传入的参数一摸一样。但是求解在内部重排序的结果却不一样。

我检查了ldd动态库链接路径，一致。最终发现有问题的程序还链接了gomp的实现，和iomp冲突了，可能就是这个导致了pardiso的内部数值问题。去掉gomp后就可以了。
